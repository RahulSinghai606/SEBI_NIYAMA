...
// LightRays — WebGL atmospheric rays (OGL), adapted for NIYAMA's light theme:
// the canvas renders with multiply blending so deep-blue rays read as
// luminous shafts over the white/sky background.

import { useRef, useEffect, useState } from "react";
import { Renderer, Program, Triangle, Mesh } from "ogl";

export type RaysOrigin = "top-center" | "top-left" | "top-right" | "left" | "right";
...
export default function LightRays({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // key light-theme adaptation: darken the white page along the rays
      ...
      const frag = `
        ...
        void main() {
          ...
          combined *= 1.2;
          // multiply-blend adaptation: output moves from white (no effect)
          // toward the ray colour where rays are strong
          vec3 tint = mix(vec3(1.0), raysColor, clamp(combined, 0.0, 1.0));
          if (noiseAmount > 0.0) {
            float n = noise(coord * 0.01 + iTime * 0.05);
            tint = mix(tint, tint * (0.96 + 0.04 * n), noiseAmount * 10.0);
          }
          if (saturation != 1.0) {
  ...
  return <div ref={containerRef} className={`absolute inset-0 w-full h-full pointer-events-none overflow-hidden ${className}`} aria-hidden />;
}
