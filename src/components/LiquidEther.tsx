...
// LiquidEther — WebGL Navier-Stokes fluid simulation (Three.js), adapted from
// the reference implementation for NIYAMA's light theme (sky/blue palette on
// transparent background). Used as the atmospheric closing-section backdrop.
...
const defaultColors = ["#0a58c4", "#2aa9e8", "#bfe3f7"];
...
export function LiquidEther({
  ...
  useEffect(() => {
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(args: any) {
        ...
        uniforms.force.value.set((Mouse.diff.x / 2) * args.mouse_force, (Mouse.diff.y / 2) * args.mouse_force);
  ...
  }, [colors.join(","), autoDemo, autoSpeed, autoIntensity, takeoverDuration, autoResumeDelay, autoRampDuration]);

  return <div ref={mountRef} className={`w-full h-full relative overflow-hidden pointer-events-none touch-none ${className}`} style={style} aria-hidden />;
}
